(function() {
  ...
  _.reject = function(collection, test) {
    ...
    var oppTest = function(x) {
      return !test(x);
    };

    return _.filter(collection, oppTest);
  };
  ...
  _.reduce = function(collection, iterator, accumulator) {
    ...
    _.each(collection, function(item) {
      callCount++;
      if (accumulator === undefined && callCount === 1) {
        accumulator = item;
      } else {
  ...
  _.zip = function() {
    ...
    var result = [];
    var maxLength = Math.max.apply(null, _.pluck(argsArray, 'length'));
    result.length = maxLength;
    result = _.map(result, function(item) {
      return [];
    });
    _.each(argsArray, function(arr) {
  ...
  _.flatten = function(nestedArray, result) {
    ...
    _.each(nestedArray, function(item) {
      if (Array.isArray(item)) {
        result = result.concat(_.flatten(item, result));
      } else {
        result.push(item);
      }
    });
